---
layout: post
title: "Media - Hack The Box"
summary: ".wmx NTLM authentication attack via Windows Media Player → responder hash capture → hash cracking → symbolic link exploitation in upload folder → web shell upload → reverse shell → FullPowers privilege escalation (NT Authority\\Local Service → SeImpersonatePrivilege) → SigmaPotato SYSTEM shell"
---
