...
summary: "Added target IP to /etc/hosts with loly.lc domain to bypass WordPress redirection, brute-forced WordPress credentials, accessed admin panel, discovered AdRotate plugin vulnerable to file upload, uploaded PHP reverse shell via ZIP, gained www-data shell, found loly credentials in wp-config.php, switched to loly user, compiled and executed kernel exploit for root."
---
