---
layout: post
title: "Baby - Hack The Box"
summary: "Anonymous LDAP enumeration → default password discovery → default user enumeration (SamAccountName missing) → smbpasswd password update → Evil-WinRM access → SeBackupPrivilege exploitation via k4sth4 repository → ntds.dit + SYSTEM file exfiltration → impacket secretsdump (Administrator hash) → impacket psexec SYSTEM shell"
---
