...
summary: "Website SQL Injection → File not found error → file parameter Local File Inclusion (LFI) → Port Knocking to open SSH port (/etc/knockd.conf) → hydra ssh brute force → SSH User shell → sudo NOPASSWD on test binary → overwrite /etc/sudoers → root"
---
