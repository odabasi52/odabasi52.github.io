...
summary: "Generated custom username wordlist from company members discovered on website, performed AS-REP Roasting on valid users to capture encrypted TGTs, cracked TGT to recover plaintext password, gained user shell, discovered service account with DCSync permissions and AutoLogon enabled via WinPEAS, extracted AutoLogon credentials from registry, performed DCSync with secretsdump.py to extract Administrator NTLM hash, gained admin shell via pass-the-hash with PsExec."
---
