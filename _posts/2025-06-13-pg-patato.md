---
layout: post
title: "Patato - OffSec Proving Grounds"
summary: "Found admin login page via Dirb, enabled FTP anonymous login revealing index.php.bak backup file, exploited PHP type juggling vulnerability (array to strcmp() returns false), bypassed authentication, extracted plaintext password from cookie, discovered path traversal in log file_name parameter to read /etc/passwd, cracked md5crypt hash with Hashcat, discovered sudo /bin/nice privilege in /notes/* directory, exploited path traversal in sudo command to execute /bin/bash for root."
---
