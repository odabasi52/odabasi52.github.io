...
summary: "Web Login Page → username admin → FTP Anonymous Login → index.php backup file → PHP Type Juggling → Authentication Bypass → file_name parameter vulnerable to Path Traversal → /etc/passwd to crack the hashes → user shell → sudo -l → sudo /bin/nice → root"
---
